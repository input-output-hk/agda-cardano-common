...
-- SESSION-51: the same selection for `driverExpose`'s two VALUE step data
pickLegVal : (l : TwoLegs) (s s′ : SysState) {X : Set 0ℓ} {e : Net_Api Payload X} {a : X}
           → LegValStep legBD s s′ e a → LegValStep legCD s s′ e a
           → LegValStep l s s′ e a
pickLegVal legBD s s′ vBD vCD = vBD
pickLegVal legCD s s′ vBD vCD = vCD
...
reach-ev-driver : (l : TwoLegs) (r : RState)
  ...
  → Σ[ r′ ∈ RState ] (M ≡ radec r′) × LegDriverStep l (toSys r) (toSys r′) e a
      ...
      × LegValStep l (toSys r) (toSys r′) e a
...
...   | s′ , medEq , N₁≡ , cWeakRun , ldBD , ldCD , sBD , sCD , lvBD , lvCD =
        r′ , Mr , pickLeg l (toSys r) s′ ldBD ldCD , pickLegSrv l (toSys r) s′ sBD sCD
      , pickLegVal l (toSys r) s′ lvBD lvCD
  where
...
evStepS-api l r aic apimem step =
  let (r′ , Mr , ld , (ue , de) , _) = reach-ev-driver l r aic apimem step
  in  r′ , Mr
