...
system : Block₃ → PTree (Net_Api Payload) (ExtI (Net_Api Payload)) (⊤ {0ℓ})
system blkA = (NetworkA ∥⇘ ioES ⇙ (nodeA blkA ⦀ (nodeB ⦀ (nodeC ⦀ nodeD)))) ∖ ioES

-- the same nodes (A producing `blkA`) over the FD-equivalent CopySpec medium (NetworkA ≈FD CopySpec)
System_CopySpec : Block₃ → PTree (Net_Api Payload) (ExtI (Net_Api Payload)) (⊤ {0ℓ})
System_CopySpec blkA = (CopySpecA ∥⇘ ioES ⇙ (nodeA blkA ⦀ (nodeB ⦀ (nodeC ⦀ nodeD)))) ∖ ioES
```
