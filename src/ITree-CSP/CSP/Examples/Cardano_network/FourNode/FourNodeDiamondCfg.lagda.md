# Four-node diamond — non-uniform config (CS+BF only)
...
Same scenario as `FourNodeDiamond`, but with a **non-uniform, trimmed**
`linkConfig`: each of the four links carries only ChainSync + BlockFetch (in
both directions) — KeepAlive/TxSubmission/Leios are unconfigured, so no peers
or medium cells are instantiated for them. `A` produces a block; it is
announced (ChainSync) and served (BlockFetch) to `B` and `C`, which relay it
onward to `D`. Each node's mini-protocol peers are *API-driven*: application
logic engages their `apiCS`/`apiBF` events to advance the protocol FSMs. See
the design doc
...
module CSP.Examples.Cardano_network.FourNode.FourNodeDiamondCfg where
```
...
Four TCP links, each running only ChainSync + BlockFetch in both directions
(KeepAlive/TxSubmission/Leios omitted — no peers for them in this scenario):

```agda
-- CS + BF, both directions; KeepAlive/TxSubmission/Leios omitted
csbfCfg : List (Dir × IDs)
csbfCfg = (lo , N2N_ChainSync)  ∷ (hi , N2N_ChainSync)
        ∷ (lo , N2N_BlockFetch) ∷ (hi , N2N_BlockFetch) ∷ []

-- per-link config: all four links carry only CS+BF
linkCfg : Fin 4 → List (Dir × IDs)
linkCfg _ = csbfCfg
```

Concrete scenario: 4 links, trimmed CS+BF-only config on each; `Block = Block₃`, other domains ⊤:
...
open import CSP.Examples.Cardano_network.NetworkPar p using (nodeBundle)
...
Sanity check: every link's configuration is the trimmed CS+BF-only `csbfCfg`
(cheap `refl`, `linkCfg` is a constant function):

```agda
-- sanity: linkAB's configuration is the CS+BF-only csbfCfg
_ : Params.linkConfig p linkAB ≡ csbfCfg
_ = refl
```
...
nodeB = (nodeBundle linkAB hi lo ⦀ nodeBundle linkBD lo hi)
...
nodeC = (nodeBundle linkAC hi lo ⦀ nodeBundle linkCD lo hi)
...
nodeD = (nodeBundle linkBD hi lo ⦀ nodeBundle linkCD hi lo)
...
An explicit LTS has-trace witness (`System_CopySpecCfg ⟹⟨ s ⟩ P′`) showing that a
