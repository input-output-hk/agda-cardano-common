...
| `TxSidePriOneLink.agda` | 237 | The **priority properties**, on a minimal fully-concrete `Params` (one link running exactly BlockFetch + LeiosFetch) so the priority decision computes by reduction: P1 (prune-under-contention), P2 (delayed-not-lost), P3 (priority introduces no deadlock). |
