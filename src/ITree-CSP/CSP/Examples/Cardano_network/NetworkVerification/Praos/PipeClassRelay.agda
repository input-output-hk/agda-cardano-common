...
------------------------------------------------------------------------
-- Praos Phase-2 R3 — the GENUINE Fix/Adv RELAY CLASSIFIER
-- (`Praos.PipeClassRelay`).
--
-- `PipeExposeNodeRelay` built a PERMISSIVE relay report (always an advance
-- from the TOTAL `allRelayAdv-of`).  This module reshapes it into the
-- `WalkDExpose.DReport` SHAPE, using the genuine classes `PipeNodeFixApi.
-- AllRelayClass` — `rcFix` (a propositional `≡`, the datum a `PipeStep⁺` relay
-- core needs on a FIXED relay) or `rcAdv` (a concrete `RelayAdv`).
--
-- CHEAPER than the client (session-12/13): a relay `cp-B`/`cp-C` is a DRIVER, so
-- its io-sync FIXITY is ALREADY surfaced by `WalkConvNodeFix.top-nodes-io-abs-fix`
-- (the six driver fixities as `≡`) — the τ-run reflector reads `rcFix` directly
-- via the thin `top-nodes-io-abs-relay-cls`, NO per-peer re-mirror.  The visible
-- api-CSBF middle moves a relay only when node B/C fires; that decision comes off
-- the SHARED api cone `top-nodes-abs-expose-cls` (which carries `AllRelayClass`).
--
...
module CSP.Examples.Cardano_network.NetworkVerification.Praos.PipeClassRelay (blkA : Block₃) where
...
------------------------------------------------------------------------
-- MEDIUM-τ reflector: a medium drain moves NO node, so every relay is FIXED.
------------------------------------------------------------------------

τreflect-med-classrelay : (r : RState) {M M′ : NetProc}
    (ms  : decMed (med (toSys r)) ─[ τ ]─► M′)
    (Meq : M ≡ ((M′ ∥⇘ ioES ⇙ absNodesOf (toSys r)) ∖ ioES))
  → Σ[ r′ ∈ RState ] (M ≡ radec r′) × AllRelayClass (toSys r) (toSys r′)
τreflect-med-classrelay r {M} {M′} ms Meq with medium-τ-inv-wt (med (toSys r)) ms
... | i , d₀ , id₀ , _ , _ , M′≡ = r′ , Meq′ , allRelayClass-refl (toSys r)
  where
    m′ : MedState
...
-- relay is `pcFix` (read off `top-nodes-io-abs-relay-cls`).
------------------------------------------------------------------------

τreflect-io-classrelay : (r : RState) {X : Set 0ℓ} {e : Net_Api Payload X} {a : X} {M₁ N₁ M : NetProc}
    (iomem : ioES .mem (X , e) a)
    (sM : decMed (med (toSys r)) ─[ ev (evl (evLabel X e a)) ]─► M₁)
    (sN : absNodesOf (toSys r)   ─[ ev (evl (evLabel X e a)) ]─► N₁)
    (Meq : M ≡ ((M₁ ∥⇘ ioES ⇙ N₁) ∖ ioES))
  → Σ[ r′ ∈ RState ] (M ≡ radec r′) × AllRelayClass (toSys r) (toSys r′)
τreflect-io-classrelay r {X} {e} {a} {M₁} {N₁} {M} iomem sM sN Meq
    with medium-ev-inv-wt (med (toSys r)) iomem sM
       | top-nodes-io-abs-relay-cls (toSys r) iomem sN
...
τreflect-classrelay : (r : RState) {M : NetProc} → radec r ─[ τ ]─► M
  → Σ[ r′ ∈ RState ] (M ≡ radec r′) × AllRelayClass (toSys r) (toSys r′)
τreflect-classrelay r step with reflect-absDec-τ (toSys r) step
... | innerτ P′ innerStep Peq
    with reflect-inner-τ (decMed (med (toSys r))) (absNodesOf (toSys r)) innerStep
...   | medτ   M′ ms eqP = τreflect-med-classrelay r ms (trans Peq (cong (λ z → z ∖ ioES) eqP))
...   | nodesτ N′ ns eqP = ⊥-elim (absNodesOf-no-τ (toSys r) ns)
τreflect-classrelay r step | hidSync M₁ N₁ iomem sM sN Peq = τreflect-io-classrelay r iomem sM sN Peq

------------------------------------------------------------------------
-- τ-run lift: fold the per-τ classifiers by `allRelayClass-trans`.
------------------------------------------------------------------------

liftτ*-classrelay′ : (r : RState) {start u : NetProc} → start ≡ radec r
          → start ─[τ*]─► u
          → Σ[ r′ ∈ RState ] (u ≡ radec r′) × AllRelayClass (toSys r) (toSys r′)
liftτ*-classrelay′ r eq τ*-refl = r , eq , allRelayClass-refl (toSys r)
liftτ*-classrelay′ r eq (τ*-step s rest) with τreflect-classrelay r (subst (λ z → z ─[ τ ]─► _) eq s)
... | r₁ , eq₁ , rep₁ with liftτ*-classrelay′ r₁ eq₁ rest
...   | r′ , equ , rep′ =
        r′ , equ , allRelayClass-trans (toSys r) (toSys r₁) (toSys r′) rep₁ rep′

liftτ*-classrelay : (r : RState) {u : NetProc}
         → radec r ─[τ*]─► u
         → Σ[ r′ ∈ RState ] (u ≡ radec r′) × AllRelayClass (toSys r) (toSys r′)
liftτ*-classrelay r = liftτ*-classrelay′ r refl

------------------------------------------------------------------------
-- VISIBLE api-CSBF MIDDLE: the relay classes come off the shared api cone
-- `top-nodes-abs-expose-cls` (node A fires ⇒ its relay advances; else fixed).
------------------------------------------------------------------------

reach-ev-classrelay : (r : RState) {X : Set 0ℓ} {e : Net_Api Payload X} {a : X} {M : NetProc}
  → IsApiCSBF e → apiES .mem (X , e) a
  → radec r ─[ ev (evl (evLabel X e a)) ]─► M
  → Σ[ r′ ∈ RState ] (M ≡ radec r′)
      × (rdec r ═[ ev (evl (evLabel X e a)) ]═► rdec r′)
      × (μTot (toSys r′) < μTot (toSys r)) × AllRelayClass (toSys r) (toSys r′)
reach-ev-classrelay r {X} {e} {a} aic apimem step
...
...   | s′ , medEq , N₁≡ , cWeakRun , drop , _ , _ , _ , relaycls =
        r′ , Mr , wr , drop , relaycls
  where
...
liftReach-classrelay : (r : RState) {X : Set 0ℓ} {e : Net_Api Payload X} {a : X} {t′ : NetProc}
  → IsApiCSBF e → apiES .mem (X , e) a
  → radec r ═[ ev (evl (evLabel X e a)) ]═► t′
  → Σ[ r′ ∈ RState ] (t′ ≡ radec r′) × AllRelayClass (toSys r) (toSys r′)
liftReach-classrelay r {X} {e} {a} aic apimem (wev pre mid post) with liftτ*-classrelay r pre
... | r₁ , eq₁ , pc₁
    with reach-ev-classrelay r₁ aic apimem
           (subst (λ z → z ─[ ev (evl (evLabel X e a)) ]─► _) eq₁ mid)
...   | r₂ , eq₂ , _ , _ , rcmid
      with liftτ*-classrelay r₂ (subst (λ z → z ─[τ*]─► _) eq₂ post)
...     | r′ , equ , pc₃ =
          r′ , equ ,
          allRelayClass-trans (toSys r) (toSys r₁) (toSys r′) pc₁
            (allRelayClass-trans (toSys r₁) (toSys r₂) (toSys r′) rcmid pc₃)

------------------------------------------------------------------------
-- The leg-`l` `RelayReport⁺`: the leg's relay CLASSIFIED across a move.
------------------------------------------------------------------------

-- how the leg-`l` relay is classified across a step/run
RelayReport⁺ : TwoLegs → SysState → SysState → Set
RelayReport⁺ l s s′ = RelayClass1 (relayOf l s) (relayOf l s′)

-- project the whole-nodes classifier onto the leg relay
allRelayClass⇒RelayReport⁺ : (l : TwoLegs) (s s′ : SysState)
                         → AllRelayClass s s′ → RelayReport⁺ l s s′
allRelayClass⇒RelayReport⁺ legBD s s′ (u , v) = u
allRelayClass⇒RelayReport⁺ legCD s s′ (u , v) = v

-- HEADLINE (τ-run half)
liftτ*-relayReport⁺ : (l : TwoLegs) (r : RState) {u : NetProc}
                  → radec r ─[τ*]─► u
                  → Σ[ r′ ∈ RState ] (u ≡ radec r′) × RelayReport⁺ l (toSys r) (toSys r′)
liftτ*-relayReport⁺ l r run with liftτ*-classrelay r run
... | r′ , eq , acc = r′ , eq , allRelayClass⇒RelayReport⁺ l (toSys r) (toSys r′) acc

-- HEADLINE (full weak move)
liftReach-relayReport⁺ : (l : TwoLegs) (r : RState)
    {X : Set 0ℓ} {e : Net_Api Payload X} {a : X} {t′ : NetProc}
  → IsApiCSBF e → apiES .mem (X , e) a
  → radec r ═[ ev (evl (evLabel X e a)) ]═► t′
  → Σ[ r′ ∈ RState ] (t′ ≡ radec r′) × RelayReport⁺ l (toSys r) (toSys r′)
liftReach-relayReport⁺ l r aic apimem w with liftReach-classrelay r aic apimem w
... | r′ , eq , acc = r′ , eq , allRelayClass⇒RelayReport⁺ l (toSys r) (toSys r′) acc
