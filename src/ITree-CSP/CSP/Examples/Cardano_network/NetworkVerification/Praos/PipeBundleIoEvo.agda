...
module CSP.Examples.Cardano_network.NetworkVerification.Praos.PipeBundleIoEvo (blkA : Block₃) where
...
open import CSP.Examples.Cardano_network.NetworkVerification.Praos.PipeBundleEvo blkA using
  ( BFsHasBlk )
open import CSP.Examples.Cardano_network.NetworkVerification.Praos.PipeFillSource blkA using
  ( PlIsBlk )
import CSP.Examples.Cardano_network.NetworkVerification.Praos.PipeCliIoDec blkA as CLI
open CLI using ( BfcIoSucc; decBFc-sendBF-succ; decBFc-receiveBF-succ )
import CSP.Examples.Cardano_network.NetworkVerification.Praos.PipeSrvIoDec blkA as SRV
...
-- block) is `⊥`.
BlkReadAt : (l : Link) (d : Dir) {X : Set 0ℓ} → Net_Api Payload X → X → Set
BlkReadAt l d (output l′ d′ N2N_BlockFetch)   a = (l′ ≡ l) × (d′ ≡ d) × PlIsBlk a
BlkReadAt l d (output _  _  N2N_ChainSync)    _ = ⊥
BlkReadAt l d (output _  _  N2N_TxSubmission) _ = ⊥
BlkReadAt l d (output _  _  N2N_KeepAlive)    _ = ⊥
BlkReadAt l d (output _  _  N2N_LeiosNotify)  _ = ⊥
BlkReadAt l d (output _  _  N2N_LeiosFetch)   _ = ⊥
BlkReadAt l d (input  _ _ _) _ = ⊥
BlkReadAt l d (sndmsg _ _ _) _ = ⊥
BlkReadAt l d (rcvmsg _ _ _) _ = ⊥
BlkReadAt l d (tx     _ _ _) _ = ⊥
BlkReadAt l d (sndack _ _ _) _ = ⊥
BlkReadAt l d (rcvack _ _ _) _ = ⊥
BlkReadAt l d (ack    _ _ _) _ = ⊥
BlkReadAt l d (done   _ _ _) _ = ⊥
BlkReadAt l d (apiCS  _ _ _) _ = ⊥
BlkReadAt l d (apiBF  _ _ _) _ = ⊥
BlkReadAt l d (apiTS  _ _ _) _ = ⊥
BlkReadAt l d (apiKA  _ _ _) _ = ⊥
BlkReadAt l d (apiLN  _ _ _) _ = ⊥
BlkReadAt l d (apiLF  _ _ _) _ = ⊥
BlkReadAt l d (break  _)     _ = ⊥
...
data BundleGEvRio⁺ (l : Link) (cl sv : Dir)
  ...
  bgEBio⁺ : (csc′ : CScPos) (css′ : CSsPos) (bfc′ : BFcPos) (bfs′ : BFsPos) (ip′ : InertPos)
       ...
       → ((bfc ≡ bfc′) ⊎ ((BFcHasBlk bfc′ → ⊥) ⊎ BlkReadAt l cl e a))
...
bfEvRio→g : {l : Link} {cl sv : Dir} {csc : CScPos} {css : CSsPos} {bfc : BFcPos} {bfs : BFsPos} {ip : InertPos}
  ...
  → BundleBFEvRio l cl sv csc css bfc bfs ip
      (λ z → (BFcHasBlk z → ⊥) ⊎ BlkReadAt l cl (ιBF e₁) a) (λ z → BFsHasBlk z → ⊥) e₁ a Bd′
