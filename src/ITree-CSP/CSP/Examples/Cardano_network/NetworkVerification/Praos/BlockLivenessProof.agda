...
--
-- The result is `BlockLiveness⁺` GREEN modulo the three SCAFFOLD premises
--   · `wprog` — weak-progress / enabledness (endgame A′),
--   · `pcone` — pending causal cone (oracle-scale pipeline provenance),
--   · `tprog` — abstract-τ-enabledness decision (the one obligation
--     `Realisableᴿ` needs that `absNoDiv`/the τ-free harvest do not supply).
...
module CSP.Examples.Cardano_network.NetworkVerification.Praos.BlockLivenessProof (blkA : Block₃) where
...
import CSP.Examples.Cardano_network.NetworkVerification.Praos.RealAbs blkA as RA
import CSP.Examples.Cardano_network.NetworkVerification.Praos.AbstractLive blkA as AL
open import CSP.Examples.Cardano_network.NetworkVerification.Praos.TProg blkA
  using ( tprog )
...
------------------------------------------------------------------------
-- THE SPINE — `BlockLiveness⁺` modulo the ONE remaining scaffold premise
-- `wprog`.  (`pcone` was discharged in SESSION-33: `AbstractLive` now takes its
-- `locate` from `Praos.PipeLocate`, the constructive pipeline-provenance walk.)
------------------------------------------------------------------------
...
module _
  ...
  broken-⊨ᵂ b =
    ⊨-DRWB-invariantᴿ→ (RA.realAbs tprog) (drbisim-sym sysBisim)
      (respondsAtoD-BS b) (AL.abstractLive wprog b)
