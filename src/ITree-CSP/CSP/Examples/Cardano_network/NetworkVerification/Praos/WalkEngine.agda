...
module CSP.Examples.Cardano_network.NetworkVerification.Praos.WalkEngine (blkA : Block₃) where
...
module _
  ...
  descend-acc : (r : RState) → Pr r → (w : WTrace (⊤ {0ℓ}) (radec r)) → Confᵂ w
              → Acc _<_ (μ (toSys r)) → ⟦ F_ (atom (arrivedD b)) ⟧ᵂ w
  descend-acc r pr w cf (acc rec) with dl r pr w cf
  ... | inj₁ arr             = zero , arr , λ _ ()
  ... | inj₂ (r′ , eq , lt , pr′) =
        F-suc (atom (arrivedD b)) w
          (F-transport (atom (arrivedD b)) eq (tail w)
            (descend-acc r′ pr′
...
... | r0 , eq0 , pr0 =
      F-transport (atom (arrivedD b)) eq0 (drop n tr)
