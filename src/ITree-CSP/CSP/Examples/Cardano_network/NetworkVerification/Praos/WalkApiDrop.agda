...
module CSP.Examples.Cardano_network.NetworkVerification.Praos.WalkApiDrop (blkA : Block₃) where
...
open import CSP.Examples.Cardano_network.NetworkVerification.Praos.SysIoLink3 blkA using
...
...     | bgEB csc′ css′ bfc′ bfs′ ip′ eq run =
          ndEBawt1 (SN.mkNodeD csc′ css′ bfc′ bfs′ (consD b′ cp′) (SN.NodeStateD.csC-CD nd) (SN.NodeStateD.csS-CD nd) (SN.NodeStateD.bfC-CD nd) (SN.NodeStateD.bfS-CD nd) (SN.NodeStateD.cons-CD nd) ip′ (SN.NodeStateD.inert-CD nd))
            ...
            cadv refl lblBD
  where
    -- at cp3 the BD-consume fires exactly `apiBF linkBD hi recvBFBlock` (`consD-c34-lbl`,
    -- with `sDBD` transported to the literal cp3 phase via record eta on `cons-BD nd`)
...
...     | bgEB csc′ css′ bfc′ bfs′ ip′ eq run =
          ndEBawt2 (SN.mkNodeD (SN.NodeStateD.csC-BD nd) (SN.NodeStateD.csS-BD nd) (SN.NodeStateD.bfC-BD nd) (SN.NodeStateD.bfS-BD nd) (SN.NodeStateD.cons-BD nd) csc′ css′ bfc′ bfs′ (consD b′ cp′) (SN.NodeStateD.inert-BD nd) ip′)
            ...
            cadv refl lblCD
  where
    -- at cp3 the CD-consume fires exactly `apiBF linkCD hi recvBFBlock`
