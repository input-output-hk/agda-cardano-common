...
module CSP.Examples.Cardano_network.NetworkVerification.Praos.WalkReachExpose (blkA : Block₃) where
...
open import CSP.Examples.Cardano_network.NetworkVerification.Praos.WalkBreakReach blkA
  using ( reach-break-μ )
...
compose-DReport s s₁ s₂ s′ (bpre , cpre) (dBD adv cmid lblmid) (bpost , cpost) =
  ...
  dBD (subst₂ ConsAdv (cong cph bpre) (cong cph (sym bpost)) adv)
      ...
      (λ hcp → lblmid (trans (cong cph bpre) hcp))
compose-DReport s s₁ s₂ s′ (bpre , cpre) (dCD adv bmid lblmid) (bpost , cpost) =
  dCD (subst₂ ConsAdv (cong cph cpre) (cong cph (sym cpost)) adv)
      ...
      (λ hcp → lblmid (trans (cong cph cpre) hcp))
