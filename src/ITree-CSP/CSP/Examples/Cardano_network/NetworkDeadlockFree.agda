...
--
-- This module proves the single `Copy id c` buffer (the per-connection
...
-- `d₀ : Data` is an inhabitant witness: the head `Copy id c` offers `input id c ? d`
...
module CSP.Examples.Cardano_network.NetworkDeadlockFree
...
open import CSP.Examples.Cardano_network.Network p Data
  using (Copy; NetProc; Menu; copyMenu; CopysId; CopySpec; allIDs; Network)
...
------------------------------------------------------------------------
-- Probe A (head): `Copy id c` is react-headed and stable.
------------------------------------------------------------------------

-- The loop0 step that `Copy id c = loop0 (pchoice (copyMenu id c))` unfolds to:
...
copyStep : (id : IDs) (c : Conn id) → ⊤ {0ℓ} → PTree (Net Data) (ExtI (Net Data)) (⊤ {0ℓ} ⊎ ⊤ {0ℓ})
copyStep id c _ = pchoice (copyMenu id c) >>= λ a′ → Ret (inj₁ a′)

-- The head of a Copy buffer, before any input: `iter (copyStep id c) tt`.
copy-A : (id : IDs) → Conn id → NetProc
copy-A id c = iter (copyStep id c) tt

-- `copy-A` is definitionally Network's `Copy` (loop0 unfolds to iter of copyStep).
Copy≡ : (id : IDs) (c : Conn id) → Copy id c ≡ copy-A id c
Copy≡ id c = refl

-- Probe: copy-A's force is `react v τc` (recover v, τc by refl).
probe-A : (id : IDs) (c : Conn id)
        → Σ[ v ∈ _ ] Σ[ τc ∈ _ ] (copy-A id c .force ≡ react v τc)
probe-A id c = _ , _ , refl

-- The head's visible-offer map.
copy-A-v : (id : IDs) (c : Conn id) → _
copy-A-v id c = let (v , _ , _) = probe-A id c in v

-- The head's τ-branch map.
copy-A-τc : (id : IDs) (c : Conn id) → _
copy-A-τc id c = let (_ , τc , _) = probe-A id c in τc
...
st-A : (id : IDs) (c : Conn id) (i : AnyTypes (ExtI (Net Data))) (a : proj₁ i)
     → copy-A-τc id c i a ≡ nothing
st-A id c i a = refl
...
c≟c : {id : IDs} (c : Conn id) → (c ≟ c) ≡ yes refl
c≟c c with c ≟ c
... | yes refl = refl
... | no ¬p    = ⊥-elim (¬p refl)

-- B d: the residual after `input id c d`.  The head's `input` offer maps to
-- `Output (output id c) d Skip` (via copyMenu), which `bindV`/`iterV` wrap into the
-- `iter-bind (… >>= k₀) (copyStep id c)` below.  Transparent ⇒ its force reduces.
copy-B : (id : IDs) (c : Conn id) → Data → NetProc
copy-B id c d =
  iter-bind (Output (output id c) d Skip >>= (λ a′ → Ret (inj₁ a′))) (copyStep id c)

-- A offers `input id c` for any `d : Data`, continuing to `copy-B id c d`.
probe-A-off : (id : IDs) (c : Conn id) (d : Data)
            → copy-A-v id c (Data , input id c) d ≡ just (copy-B id c d)
probe-A-off id c d rewrite id≟id id | c≟c c = refl

-- Probe: B d's force is `react v τc` (Output is react-headed; bind preserves this).
probe-B : (id : IDs) (c : Conn id) (d : Data)
        → Σ[ v ∈ _ ] Σ[ τc ∈ _ ] (copy-B id c d .force ≡ react v τc)
probe-B id c d = _ , _ , refl

-- B d's visible-offer map.
copy-B-v : (id : IDs) (c : Conn id) (d : Data) → _
copy-B-v id c d = let (v , _ , _) = probe-B id c d in v

-- B d's τ-branch map.
copy-B-τc : (id : IDs) (c : Conn id) (d : Data) → _
copy-B-τc id c d = let (_ , τc , _) = probe-B id c d in τc
...
st-B : (id : IDs) (c : Conn id) (d : Data)
       (i : AnyTypes (ExtI (Net Data))) (a : proj₁ i)
     → copy-B-τc id c d i a ≡ nothing
st-B id c d i a = refl

-- C d: the loop-back residual after `output id c ! d`.  B's `output` offer maps to
-- `Skip` (Output-cont on the matching value), wrapped to `iter-bind (Skip >>= k₀) …`.
copy-C : (id : IDs) (c : Conn id) → Data → NetProc
copy-C id c d =
  iter-bind (Skip >>= (λ a′ → Ret (inj₁ a′))) (copyStep id c)

-- B d offers `output id c` carrying exactly `d`, continuing to `copy-C id c d`.
probe-B-off : (id : IDs) (c : Conn id) (d : Data)
            → copy-B-v id c d (Data , output id c) d ≡ just (copy-C id c d)
probe-B-off id c d rewrite id≟id id | c≟c c with d ≟ d
... | yes refl = refl
... | no ¬p    = ⊥-elim (¬p refl)

-- C is sil-headed, looping back to the head A (= Copy id c).
probe-C : (id : IDs) (c : Conn id) (d : Data)
        → copy-C id c d .force ≡ sil (copy-A id c)
probe-C id c d = refl
...
-- The three reachable control states of `Copy id c`: head A, post-input B d,
-- loop-back C d.  (B/C are indexed by the carried datum `d`.)
data CopyPos (id : IDs) (c : Conn id) : NetProc → Set where
  is-A : CopyPos id c (copy-A id c)
  is-B : (d : Data) → CopyPos id c (copy-B id c d)
  is-C : (d : Data) → CopyPos id c (copy-C id c d)

-- Every reachable state has an enabled move: A offers `input id c d₀`, B offers
-- `output id c d`, C does a τ (loop-back to A).
copyPos-move : ∀ {id c t} → CopyPos id c t
             → Σ[ l ∈ Label (⊤ {0ℓ}) ] Σ[ t″ ∈ NetProc ] (t ─[ l ]─► t″)
copyPos-move {id} {c} is-A =
  _ , _ , sVis {at = Data , input id c} {a = d₀}
                (let (_ , _ , eq) = probe-A id c in eq) (probe-A-off id c d₀)
copyPos-move {id} {c} (is-B d) =
  _ , _ , sVis {at = Data , output id c} {a = d}
                (let (_ , _ , eq) = probe-B id c d in eq) (probe-B-off id c d)
copyPos-move {id} {c} (is-C d) = _ , _ , sSil (probe-C id c d)
...
copyPos-stepτ : ∀ {id c t t″} → CopyPos id c t → t ─[ τ ]─► t″ → CopyPos id c t″
-- A: react-headed, no sil; τc ≡ nothing everywhere ⇒ no sTau.
copyPos-stepτ {id} {c} is-A (sSil eq) = case eq of λ ()
copyPos-stepτ {id} {c} is-A (sTau {i = i} {a = a} refl br) =
  case trans (sym (st-A id c i a)) br of λ ()
-- B: same (Output is a stable react node).
copyPos-stepτ {id} {c} (is-B d) (sSil eq) = case eq of λ ()
copyPos-stepτ {id} {c} (is-B d) (sTau {i = i} {a = a} refl br) =
  case trans (sym (st-B id c d i a)) br of λ ()
-- C: sil-headed, τ-steps to A.
copyPos-stepτ {id} {c} (is-C d) (sSil refl)      = is-A
copyPos-stepτ {id} {c} (is-C d) (sTau eq br)     = case eq of λ ()

-- Helper: A's `input id′ c′` offer is `just (copy-B id c a)` exactly when the event
...
A-off-pos : (id : IDs) (c : Conn id) (id′ : IDs) (c′ : Conn id′) (a : Data) {t″ : NetProc}
          → copy-A-v id c (Data , input id′ c′) a ≡ just t″ → CopyPos id c t″
A-off-pos id c id′ c′ a br with id′ ≟ id
... | no  _ = case br of λ ()
A-off-pos id c id′ c′ a br | yes refl with c′ ≟ c
... | no  _    = case br of λ ()
... | yes refl = subst (CopyPos id c) (just-injective br) (is-B a)

-- Helper (dual): B d's `output id′ c′` offer fires to `copy-C id c d` exactly when the
-- event targets this buffer AND carries the stored datum d; otherwise `nothing`.
B-off-pos : (id : IDs) (c : Conn id) (d : Data)
            (id′ : IDs) (c′ : Conn id′) (a : Data) {t″ : NetProc}
          → copy-B-v id c d (Data , output id′ c′) a ≡ just t″ → CopyPos id c t″
-- B's offer fires through `Output-cont (output id c) d Skip`, whose `Net-≟` decision
...
B-off-pos id c d id′ c′ a br with id ≟ id′
... | no  _ = case br of λ ()
B-off-pos id c d id′ c′ a br | yes refl with c ≟ c′
... | no  _    = case br of λ ()
... | yes refl with a ≟ d
... | no  _    = case br of λ ()
... | yes refl = subst (CopyPos id c) (just-injective br) (is-C d)

-- A visible step from any Copy position lands on a Copy position.  A's only offer
-- is `input id c` (→ B); B's only offer is `output id c` carrying the stored `d`
-- (→ C); C is sil-headed (no visible offer).  All other events are refused.
copyPos-stepev : ∀ {id c t e t″} → CopyPos id c t
               → t ─[ ev (evl e) ]─► t″ → CopyPos id c t″
-- A: split on the offered event; only `input id c` (with matching id/conn) fires.
copyPos-stepev {id} {c} is-A (sVis {at = _ , input id′ c′} {a = a} refl br) =
  A-off-pos id c id′ c′ a br
copyPos-stepev {id} {c} is-A (sVis {at = _ , output _ _} refl br) = case br of λ ()
copyPos-stepev {id} {c} is-A (sVis {at = _ , sndmsg _ _} refl br) = case br of λ ()
copyPos-stepev {id} {c} is-A (sVis {at = _ , rcvmsg _ _} refl br) = case br of λ ()
copyPos-stepev {id} {c} is-A (sVis {at = _ , tx _ _}     refl br) = case br of λ ()
copyPos-stepev {id} {c} is-A (sVis {at = _ , sndack _ _} refl br) = case br of λ ()
copyPos-stepev {id} {c} is-A (sVis {at = _ , rcvack _ _} refl br) = case br of λ ()
copyPos-stepev {id} {c} is-A (sVis {at = _ , ack _ _}    refl br) = case br of λ ()
-- B: only `output id c` (with matching id/conn AND stored value d) fires.
copyPos-stepev {id} {c} (is-B d) (sVis {at = _ , output id′ c′} {a = a} refl br) =
  B-off-pos id c d id′ c′ a br
copyPos-stepev {id} {c} (is-B d) (sVis {at = _ , input _ _}  refl br) = case br of λ ()
copyPos-stepev {id} {c} (is-B d) (sVis {at = _ , sndmsg _ _} refl br) = case br of λ ()
copyPos-stepev {id} {c} (is-B d) (sVis {at = _ , rcvmsg _ _} refl br) = case br of λ ()
copyPos-stepev {id} {c} (is-B d) (sVis {at = _ , tx _ _}     refl br) = case br of λ ()
copyPos-stepev {id} {c} (is-B d) (sVis {at = _ , sndack _ _} refl br) = case br of λ ()
copyPos-stepev {id} {c} (is-B d) (sVis {at = _ , rcvack _ _} refl br) = case br of λ ()
copyPos-stepev {id} {c} (is-B d) (sVis {at = _ , ack _ _}    refl br) = case br of λ ()
-- C: sil-headed, so no visible step (force ≡ sil ≠ react).
copyPos-stepev {id} {c} (is-C d) (sVis eq br) = case eq of λ ()
...
copy-live′ : ∀ {id c t} → CopyPos id c t → Live t
...
copy-live : ∀ id (c : Conn id) → Live (Copy id c)
copy-live id c = copy-live′ {id} {c} is-A
...
-- CopysId id = ⦀Fin (numConns id) (Copy id) is live.
copysId-live : ∀ id → Live (CopysId id)
copysId-live id = interFin-live (numConns id) (Copy id) (copy-live id)

-- ||| over a list of per-id buffers is live (base ⦀⋆ [] = Skip).
interList-live : (xs : List IDs) → Live (⦀⋆ (map CopysId xs))
interList-live []       = skip-live
interList-live (x ∷ xs) = ⦀-live (copysId-live x) (interList-live xs)

-- CopySpec = ⦀⋆ (map CopysId allIDs) is live.
network-live : Live CopySpec
network-live = interList-live allIDs
