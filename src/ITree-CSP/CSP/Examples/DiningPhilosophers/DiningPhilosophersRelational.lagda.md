...
process-tree / CSP semantics by a react-guarded generator `proc`.
...
- `proc-allOne-IsStuck`        — the §F process-tree bridge lifts the relational
...
analyses, and liveness) and then **Part F** — the process-tree bridge
(`§F`): the event type `DP`, the decidable enabled-step `stepTo?`, the
coinductive generator `proc` (productive via react-guarding, **no** termination
pragma), soundness/completeness of the decision, and the lifted deadlock.

```agda
{-# OPTIONS --guardedness #-}

module CSP.Examples.DiningPhilosophers.DiningPhilosophersRelational where
```

## §1. Imports

We work entirely in `Fin`/`ℕ` and propositional equality; no process
...
## §F. The process-tree bridge

So far the model is a *relational* transition system on `Config`. We now
build the bridge to the process-tree / CSP semantics: a coinductive
`proc : Config → PTree DP …` whose visible events are exactly the enabled
...
actions as visible events. The continuation `proc c'` sits under the `just`
of the `react` node's visible-offer map, so `proc` is productive *without*
any termination pragma — it is react-guarded. The `react` node carries an
everywhere-`nothing` τ-branch map, so it is **stable** (no τ-move).
...
```agda
  ...
  -- sits literally under the `just` of the `react` visible-offer map —
  -- react-guarded, hence productive without any termination pragma.
  procStep : Maybe Config → Maybe (PTree DP (ExtI DP) (⊤ {lzero}))
  proc     : Config → PTree DP (ExtI DP) (⊤ {lzero})

  procStep nothing   = nothing
  procStep (just c') = just (proc c')

  PTree.force (proc c) =
    react (λ where (_ , act i k) tt → procStep (stepTo? c i k))
          (λ _ _ → nothing)
```
...
```agda
  ...
  -- force (proc c) = react, a stable node (its τ-map is everywhere nothing),
  -- so the only inbound LTS constructor that can fire is sVis (a visible
  -- `act i k`); sRet/sSil are refuted by force ≢ ret/sil and sTau by the
  -- everywhere-nothing τ-map. An sVis firing carries
  -- eqj : procStep (stepTo? c i k) ≡ just t', forcing stepTo? c i k ≡ just c'';
  -- stepTo?-sound then gives an enabled action, contradicting Stuck c.
  proc-stuck : ∀ {c} → Stuck c → IsStuck (proc c)
  proc-stuck {c} stk (sRet eq)  = case eq of λ ()
  proc-stuck {c} stk (sSil eq)  = case eq of λ ()
  proc-stuck {c} stk (sTau refl br) = case br of λ ()
...
### §10.5. The deadlock as a genuine process-tree `IsStuck`

The relational deadlock of the symmetric system (`Sym.Stuck Sym.allOne`)
lifts, through the §F bridge, to a real process-tree deadlock: the
