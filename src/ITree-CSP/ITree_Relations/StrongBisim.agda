...
data SNodeKindF {ℓ ℓe ℓi ℓr ℓ≡ ℓ≈ : Level}
  ...
  invF : ∀ {f₁ f₂ i₁ i₂ a₁ a₂ p₁ p₂}
       ...
       → (∀ {t₂} → Image f₂ t₂
          → Σ[ t₁ ∈ ITree E I R ] (Image f₁ t₁ × TreeRel t₁ t₂))
       → SNodeKindF RetRel TreeRel (inv f₁ i₁ a₁ p₁) (inv f₂ i₂ a₂ p₂)
...
-- Extract matching inv continuation from a bisimulation
sbisim-inv : ∀ {ℓ ℓe ℓi ℓr} {E : Set ℓ → Set ℓe} {I : Set ℓ → Set ℓi} {R : Set ℓr}
               {t t' : ITree E I R}
               {f₁ : (i : AnyTypes I) → ContinueType i (Maybe (ITree E I R))}
               {i₁ : AnyTypes I} {a₁ : proj₁ i₁} {p₁ : Is-just (f₁ i₁ a₁)}
           → t ≈ t'
           → ITree.force t ≡ inv f₁ i₁ a₁ p₁
           → Σ[ f₂ ∈ ((i : AnyTypes I) → ContinueType i (Maybe (ITree E I R))) ]
             ...
             × (∀ {t₂} → Image f₂ t₂ → Σ[ t₁ ∈ ITree E I R ] (Image f₁ t₁ × t₁ ≈ t₂)) )
sbisim-inv {t = t} {t' = t'} {f₁ = f₁} {i₁ = i₁} {a₁ = a₁} {p₁ = p₁} bisim eq
  with ITree.force t | ITree.force t' | Sbisim.step bisim
... | w | w' | step-val 
  with w | eq
... | .(inv f₁ i₁ a₁ p₁) | refl 
...
... | .(inv f₁ i₁ a₁ p₁) | inv f₂ i₂ a₂ p₂ | invF fwd bwd | refl
    = f₂ , i₂ , a₂ , p₂ , refl , fwd , bwd        

sbisim-inv-fwd : ∀ {ℓ ℓe ℓi ℓr} {E : Set ℓ → Set ℓe} {I : Set ℓ → Set ℓi} {R : Set ℓr}
                   {t t' t₁ : ITree E I R}
                   {f₁ : (i : AnyTypes I) → ContinueType i (Maybe (ITree E I R))}
                   {i₁ : AnyTypes I} {a₁ : proj₁ i₁} {p₁ : Is-just (f₁ i₁ a₁)}
               → t ≈ t'
...
sbisim-inv-fwd {t = t} {t' = t'} {f₁ = f₁} {i₁ = i₁} {a₁ = a₁} {p₁ = p₁} bisim eq im
    with ITree.force t | ITree.force t' in eq-t' | Sbisim.step bisim | eq
... | .(inv f₁ i₁ a₁ p₁) | inv f₂ i₂ a₂ p₂ | invF fwd _ | refl
    with fwd im
... | t₂ , (i' , a' , wit₂) , sim
    = t₂ , sInv eq-t' wit₂ , sim
...
module SbisimEquiv
  ...
  ... | inv _ _ _ _ | inv _ _ _ _ | invF h-fwd h-bwd = invF
        (λ {t₁} img →
  ...
  ... | inv _ _ _ _ | inv _ _ _ _ | inv _ _ _ _  | invF hp-fwd hp-bwd  | invF hq-fwd hq-bwd = invF
        (λ {t₁} img →
...
traces-fwd bisim (bTau (sInv {i = i} {a = a} eq wit) rest)
  with sbisim-inv-fwd bisim eq (i , a , wit)
... | t₂' , step₂ , sim
  with traces-fwd sim rest
... | t''' , steps , bisim'
