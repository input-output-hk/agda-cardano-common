...
... | vis fP = step .force
  ...
  {- ▷ inv (λ ai a →                  -- ⨅ a : A ∩ cs @ (P[a/x] ∖ cs)
    case fP (extE ai) a of λ where     -- ExtI lifts hidden E-events into I
      nothing  → nothing
      (just P') → just (P' ∖ cs ¿ dec))
  -}    
{-  where
    hide-step : (at : AnyTypes E) → (ContinueType at (Maybe (ITree E R))) → NodeKind E R
    hide-step at x = case dec at of λ where
      (yes _) → case fP at x of λ where
        nothing → nothing
        (just P') → just (P' ∖ cs ¿ dec)
      (no _) → 
-}
